...
class Simulation:
    def __init__(self, params=Params):
        ...
        self.num_arrive = 0
        self.num_depart = 0
        self.num_reject = len([c for c in self.queue if not c.serviced])
        self.total_wait = sum([c.get_wait_time() for c in self.queue])
        self.num_serviced = len([c for c in self.queue if c.serviced])
    ...
    def handle_arrive_event(self):
        ...
        if len(self.queue) >= self.params.K:
            ...
            c.serviced = False
            c.priority = self.params.K-len(queue)
